...
function respondToMessage(event) {
    switch (event.name) {
        ...
        case "downloadMedia":
            ...
            prompt("Copiez l'URL ci-dessous et collez-le dans la fenêtre des Téléchargements.", event.message);
            break;
    }
}
...
function printMedia(mediaType) {
    switch(mediaType) {
        ...
        default: return "la vidéo";
    }
}

function handleContextMenu(event) {
    if(!event.userInfo.CTPInstance) {
		if(event.userInfo.blocked > 0) event.contextMenu.appendContextMenuItem("loadall", "Débloquer tous les plugins (" + event.userInfo.blocked + ")");
    ...
    if(event.userInfo.isH264) {
        ...
        //         }
        event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",download", "Télécharger " + printMedia(event.userInfo.mediaType) + "\u2026");
		if(event.userInfo.siteInfo) event.contextMenu.appendContextMenuItem("gotosite", "Voir la vidéo sur " + event.userInfo.siteInfo.name);
        ...
        if(event.userInfo.hasH264) {
            event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",plugin", "Lancer " + event.userInfo.plugin);
            event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",video", "Lancer " + printMedia(event.userInfo.mediaType));
            event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",download", "Télécharger " + printMedia(event.userInfo.mediaType) + "\u2026");
			if(event.userInfo.siteInfo) event.contextMenu.appendContextMenuItem("gotosite", "Voir la vidéo sur " + event.userInfo.siteInfo.name);
        } else {
            event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",plugin", "Lancer le plugin");
        }
        if(safari.extension.settings["useWLcontext"]) {
            event.contextMenu.appendContextMenuItem("srcwhitelist", "Ajouter à la liste blanche\u2026");
        }
        event.contextMenu.appendContextMenuItem(event.userInfo.CTPInstance + "," + event.userInfo.elementID + ",remove", "Supprimer l'élément");
        // BEGIN DEBUG
