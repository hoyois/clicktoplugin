		...
		case "loadContent":
			...
			switch(event.message.command) {
				case "plugin":
					...
					break;
				case "changeLabel":
					if(placeholderElements[event.message.elementID] && !mediaPlayers[event.message.elementID]) displayBadge("Flash", event.message.elementID);
					break;
				case "remove":
...
function handleBeforeLoadEvent(event) {
	...
	// Display the badge
	displayBadge(responseData.unknownType ? "?" : "Flash", elementID);
	
	if(responseData.unknownType) safari.self.tab.dispatchMessage("checkMIMEType", {"documentID": documentID, "elementID": elementID, "url": data.src});
...
function prepMedia(mediaData) {
	...
	else if(hasSourceSelector) displayBadge("Flash*", elementID);
}
